...
if [ "$MODE_DEV_PROD" = "PROD" ]; then
  ...
  jupyter lab --no-browser --ip=0.0.0.0 --port=8888 & tensorboard --bind_all --logdir /srv/notebooks/logs/ & mlflow ui -p 6000 -h 0.0.0.0 & uvicorn app.main:app --host 0.0.0.0 --port 5000 --workers "$num_workers" --reload-dir app $uvicorn_https_param
fi